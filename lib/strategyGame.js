...
export class StrategyGame {
  constructor() {
    ...
    this.tileSize =
      this.NATIVE_WIDTH / ((constants.WIDTH - 1.1) * constants.HEX_WIDTH_RATIO)
    const TILE_HEIGHT = this.tileSize * constants.HEX_HEIGHT_RATIO
    this.SCALE = this.NATIVE_WIDTH / 1280
    this.OFFSET_X = -this.tileSize
    this.OFFSET_Y = (this.NATIVE_HEIGHT - TILE_HEIGHT * constants.HEIGHT) / 2.5
  ...
  canMoveUnit = ({ playerId, unitId, x, y }) => {
    ...
    return unit?.team === player?.team && !this.hexes.isOccupied(hex, unit)
  }
  ...
  spawn = ({ playerId }) => {
    ...
    const hex = constants.UNITS.find((coords) => {
      const targetHex = this.hexes.get({ x: coords.gridX, y: coords.gridY })
      return coords.team === player.team && !targetHex.unit
    })
    if (hex && playerUnitCount < UNIT_CAP) {
      player.money -= UNIT_COST
      this.units = [...this.units, this.getNewUnit(hex)]
    }
  }
  ...
  _attackTarget = (unit) => {
    ...
    const target = this.hexes.hexGrid
  ...
  _getCharge = () => {
    ...
    if (this.charge >= 99) {
      const otherPlayer = this.players.find((p) => p.team !== this.chargeIndex)
