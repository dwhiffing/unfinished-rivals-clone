...
export class UnitManager {
  ...
  _moveTowardDestination = (unit) => {
    const currentHex = this.strategyGame.hexes.getHexFromScreen({
      x: unit.x,
      y: unit.y,
    })
    ...
    if (
      ...
      unit.path = this.strategyGame.hexes.getPath(
        unit,
        unit.hex,
        this.strategyGame.hexes.getNearestNeighbour(currentHex, unit),
      )
    }

    const currentDestination = unit.path[0]
    const { x, y } = this.strategyGame.hexes.getScreenFromHex(
      currentDestination,
    )
    if (
