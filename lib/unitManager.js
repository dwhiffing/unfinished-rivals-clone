...
export class UnitManager {
  ...
  _attackTarget = (unit) => {
    ...
    unit.target = target.unit.id
    unit.fireRateCounter -= 1
    if (unit.fireRateCounter > 0) return
    unit.fireRateCounter = unit.fireRate
  ...
  _moveTowardDestination = (unit) => {
    ...
    if (unit.gridX !== currentHex.x || unit.gridY !== currentHex.y) {
      ...
      // TODO: this still fails if 2 units enter the same time at around the same time
      const unitInNextHex = this.getUnitForHex(nextHex)
      if (
        unitInNextHex &&
        unitInNextHex.id !== unit.id &&
        (unitInNextHex.team !== unit.team ||
          (unit.path.length === 1 &&
            unitInNextHex.path.length === 0 &&
            unitInNextHex.team === unit.team))
      ) {
